...
export function WeatherWidgetLarge() {
  ...
  return (
    <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-900 rounded-2xl p-8 text-white shadow-2xl">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Seção Esquerda - Clima Atual */}
        <div className="lg:col-span-1 flex flex-col justify-center">
          ...
          <div className="flex items-end gap-4">
            <div>
              ...
              <p className="text-blue-200 text-sm mt-1">Sensação: {weather.feelsLike}°C</p>
              <p className="text-sm mt-2 capitalize text-blue-100">{weather.description}</p>
            </div>
            <div className="text-right mb-1">
              {weatherIcons[weather.icon] || <Cloud className="w-16 h-16" />}
            </div>
          </div>
        </div>

        {/* Seção Centro - Dados Atuais */}
        <div className="lg:col-span-1">
          <div className="grid grid-cols-2 gap-3 h-full">
            <div className="bg-white/10 rounded-lg p-4 flex flex-col items-center justify-center">
              ...
              <p className="text-blue-200 text-xs mb-1">Umidade</p>
              <p className="font-bold text-xl">{weather.humidity}%</p>
            </div>
            <div className="bg-white/10 rounded-lg p-4 flex flex-col items-center justify-center">
              <Wind className="w-6 h-6 text-gray-300 mb-2" />
              <p className="text-blue-200 text-xs mb-1">Vento</p>
              <p className="font-bold text-xl">{weather.windSpeed} km/h</p>
            </div>
            <div className="bg-white/10 rounded-lg p-4 flex flex-col items-center justify-center">
              <CloudRain className="w-6 h-6 text-blue-300 mb-2" />
              <p className="text-blue-200 text-xs mb-1">Chuva</p>
              <p className="font-bold text-xl">{weather.rain} mm</p>
            </div>
            <div className="bg-white/10 rounded-lg p-4 flex flex-col items-center justify-center">
              <Sun className="w-6 h-6 text-yellow-400 mb-2" />
              <p className="text-blue-200 text-xs mb-1">UV</p>
              <p className="font-bold text-xl">{weather.uvIndex}</p>
            </div>
          </div>
        </div>

        {/* Seção Direita - Previsão */}
        <div className="lg:col-span-1">
          <p className="text-sm text-blue-200 mb-3 font-semibold">Previsão dos próximos dias</p>
          <div className="space-y-2">
            {forecast.map((day, idx) => (
              <div key={idx} className="flex items-center justify-between bg-white/5 rounded-lg p-3">
                <p className="text-xs text-blue-200 min-w-16">{getDayName(day.date)}</p>
                ...
                <p className="text-xs font-bold min-w-14 text-right">
                  {day.maxTemp}° / {day.minTemp}°
                </p>
                {day.chanceOfRain > 0 && (
                  <p className="text-xs text-cyan-300 min-w-12 text-right">
                    {day.chanceOfRain}%
                  </p>
                )}
              </div>
